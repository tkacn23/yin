...
def main(audioFileName="6.wav", w_len=2056, w_step=256, f0_min=50, f0_max=600, harmo_thresh=0.85, audioDir="./", dataFileName=None, verbose=4):
    """
    ...
    if verbose >3:
        ...
        for k in range(1,5) :
            for l in range (2,6) :
                if k >= l or (k==2 and l==4) : continue
                if d // k == c // l or d // k == (c// l) + 1  :
                    e = k
                    f = l      
        print (e)
        print (f)    
        for j in range(0, len(pitches)) :
            if argmins[j] == 0 : argmins[j] = 0
            elif argmins[j] == c : argmins[j] = float(sr/(argmins[j]/ f))
            elif argmins[j] == d : argmins[j] = float(sr/(argmins[j]/ e))
            else : argmins[j] = float(sr/argmins[j])
